...
impl I18nManager {
    ...
    /// 从文件加载语言资源
    pub fn load_from_file(&mut self, language: NaturalLanguage, path: &str) -> Result<(), Box<dyn std::error::Error>> {
...
pub static I18N_MANAGER: Lazy<RwLock<I18nManager>> = Lazy::new(|| {
    ...
    let default_english = HashMap::from([
        ("welcome".to_string(), "Welcome to Codex AI Programming Assistant!".to_string()),
        ("enter_solo_mode".to_string(), "Entering solo mode for AI programming...".to_string()),
        ("starting_task".to_string(), "Starting solo mode for task: {0}".to_string()),
        ("max_steps".to_string(), "Maximum steps: {0}".to_string()),
        ("config_welcome".to_string(), "This is your first time running Codex. Let's configure it together!".to_string()),
        ("language_selection".to_string(), "Select Interface Language".to_string()),
        ("ai_settings".to_string(), "AI Settings".to_string()),
        ("ui_settings".to_string(), "UI Settings".to_string()),
        ("config_summary".to_string(), "Configuration Summary".to_string()),
        ("config_complete".to_string(), "Configuration Complete!".to_string()),
    ]);
    ...
    let default_chinese = HashMap::from([
        ("welcome".to_string(), "欢迎使用 Codex AI 编程助手!".to_string()),
        ("enter_solo_mode".to_string(), "正在进入 solo 模式进行 AI 编程...".to_string()),
        ("starting_task".to_string(), "开始 solo 模式，任务: {0}".to_string()),
        ("max_steps".to_string(), "最大步骤: {0}".to_string()),
        ("config_welcome".to_string(), "这是您第一次运行 Codex。让我们一起配置它!".to_string()),
