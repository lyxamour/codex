...
impl WebScraper {
    ...
    pub async fn scrape(&mut self, urls: &[String]) -> Result<Vec<(String, String)>, Box<dyn Error>> {
    ...
    fn scrape_single<'a>(&'a mut self, url: &str, depth: u32) -> Pin<Box<dyn Future<Output = Result<Vec<ScrapedContent>, Box<dyn Error>>> + 'a>> {
