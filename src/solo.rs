...
impl SoloAgent {
    ...
    async fn decompose_task(&self, task: &str, max_steps: u32) -> Result<Vec<SoloStep>, Box<dyn Error>> {
        let prompt = format!(
    ...
    async fn synthesize_result(&self, original_task: &str, results: &[String]) -> Result<String, Box<dyn Error>> {
    ...
    pub async fn execute_with_actions(&mut self, task: &str, max_steps: u32) -> Result<String, Box<dyn Error>> {
        println!("Starting enhanced solo mode with real-world actions for task: {}", task);
        println!("Maximum steps: {}", max_steps);
        ...
        let task = self.task_manager.create(
            format!("Solo Mode: {}", task),
            "high".to_string()
        )?;
        ...
        let final_result = format!("Task completed successfully. Check the task manager for details: {}", task.id);
    ...
    pub async fn execute_parallel(
        ...
        println!("Maximum steps: {}, Maximum parallel: {}", max_steps, max_parallel);
