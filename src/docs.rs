...
impl DocsGenerator {
    ...
    fn should_process_file(&self, path: &Path) -> bool {
        ...
        if let Some(ext) = path.extension() {
            ...
            match ext {
                ...
                "rs" | "py" | "js" | "ts" | "md" | "html" | "css" | "json" | "yaml" | "yml"
                | "toml" => true,
                _ => false,
            }
        } else {
    ...
    fn generate_rust_docs(&mut self, path: &Path, content: &str) -> Result<(), Box<dyn Error>> {
        ...
        let output_path = Path::new(&self.config.output_dir).join(format!("{}.md", file_name));
    ...
    fn extract_javascript_structure(
        &self,
        file: &mut File,
        _content: &str,
    ) -> Result<(), Box<dyn Error>> {
    ...
    fn extract_typescript_structure(
        &self,
        file: &mut File,
        _content: &str,
    ) -> Result<(), Box<dyn Error>> {
    ...
    fn generate_index_file(&mut self) -> Result<(), Box<dyn Error>> {
        let index_path = Path::new(&self.config.output_dir).join("index.md");
