...
impl ToolExecutor {
    /// 创建新的工具执行器实例
    pub fn new(registry: std::sync::Arc<std::sync::RwLock<crate::tools::registry::ToolRegistry>>) -> Self
    {
        Self {
            registry,
        }
    ...
    {
        // 1. 检查工具是否存在
        let registry = self.registry.read().map_err(|e| crate::error::AppError::Tool(format!("无法获取工具注册表: {}", e)))?;
    ...
    /// 验证参数
    fn validate_params(&self, tool_metadata: &crate::tools::registry::ToolMetadata, params: &serde_json::Value) -> Result<(), String>
    {
    ...
    async fn execute_read_file(&self, params: serde_json::Value) -> AppResult<ToolResult>
    {
        ...
        let path = params.get("path").and_then(|v| v.as_str()).ok_or(crate::error::AppError::Tool("缺少path参数".to_string()))?;
    ...
    async fn execute_write_file(&self, params: serde_json::Value) -> AppResult<ToolResult>
    {
        ...
        let path = params.get("path").and_then(|v| v.as_str()).ok_or(crate::error::AppError::Tool("缺少path参数".to_string()))?;
        let content = params.get("content").and_then(|v| v.as_str()).ok_or(crate::error::AppError::Tool("缺少content参数".to_string()))?;
