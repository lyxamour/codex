...
- [ ] 创建基础项目目录结构
  - [ ] 创建 src/main.rs 作为程序入口，实现 main 函数
  - [ ] 创建 src/lib.rs 作为库入口，导出公共模块
  - [ ] 创建 src/config/ 目录，包含 app.rs 和 loader.rs
  - [ ] 创建 src/error.rs 定义基本错误类型 AppError
  - [ ] 创建 src/cli/ 目录，包含 commands.rs 和 args.rs
  - [ ] 创建 src/core/ 目录，包含 app.rs 和 runtime.rs
  - [ ] 创建 src/tools/ 目录，包含 mod.rs, registry.rs, executor.rs
  - [ ] 创建 src/tools/builtin/ 目录，包含 mod.rs, file.rs, shell.rs
  - [ ] 创建 src/ai/ 目录，包含 mod.rs, adapter.rs, prompt.rs
  - [ ] 创建 ~/codex/ 目录结构
    - [ ] 创建 ~/codex/config/ 目录
    - [ ] 创建 ~/codex/data/ 目录
    - [ ] 创建 ~/codex/prompts/ 目录
    ...
    - [ ] 创建 ~/codex/logs/ 目录
- [ ] 配置 Cargo.toml 基础依赖
  - [ ] 添加 clap = "4.0" 依赖用于命令行解析
  - [ ] 添加 serde = { version = "1.0", features = ["derive"] } 用于序列化
  - [ ] 添加 tokio = { version = "1.0", features = ["full"] } 用于异步运行时
  - [ ] 添加 anyhow = "1.0" 依赖用于错误处理
  - [ ] 添加 yaml-rust = "0.4" 依赖用于YAML解析
  - [ ] 添加 dirs = "5.0" 依赖用于目录路径处理
- [ ] 实现基本命令行界面
  - [ ] 实现 "codex --version" 命令显示版本信息
  - [ ] 实现 "codex --help" 命令显示帮助信息
  - [ ] 实现基本错误处理和用户友好的错误信息

### MVP.2 基础工具系统 (2-3 天)

- [ ] 实现工具系统基础架构
  - [ ] 在 src/tools/mod.rs 中定义工具相关结构体和trait
  - [ ] 在 src/tools/registry.rs 中实现工具注册表
  - [ ] 在 src/tools/executor.rs 中实现工具执行器
  - [ ] 在 src/tools/parser.rs 中实现YAML工具定义解析器
- [ ] 实现基础内置工具
  - [ ] 在 src/tools/builtin/file.rs 中实现文件读写工具
    - [ ] 实现 read_file 工具，支持读取文件内容
    - [ ] 实现 write_file 工具，支持写入文件内容
  - [ ] 在 src/tools/builtin/shell.rs 中实现Shell命令工具
    - [ ] 实现 shell 工具，支持执行基本Shell命令
- [ ] 实现工具YAML定义
  - [ ] 创建 templates/tools/file_ops.yaml 文件操作工具定义
  - [ ] 创建 templates/tools/shell_cmd.yaml Shell命令工具定义
- [ ] 实现 "codex tool <tool_name>" 命令
  - [ ] 在 src/cli/commands.rs 中添加 tool 子命令
  - [ ] 实现工具调用处理逻辑，支持YAML格式输入
  - [ ] 实现工具结果输出和错误处理

### MVP.3 基础提示词系统 (1-2 天)

- [ ] 实现提示词系统基础架构
  - [ ] 在 src/ai/prompt.rs 中实现提示词管理器
  - [ ] 实现提示词加载和解析功能
  - [ ] 实现提示词变量替换功能
- [ ] 创建基础提示词模板
  - [ ] 创建 templates/prompts/default.yaml 默认提示词
  - [ ] 创建 templates/prompts/code.yaml 代码相关提示词
  - [ ] 创建 ~/codex/prompts/custom/ 目录用于用户自定义提示词
- [ ] 实现 "codex prompt <prompt_name>" 命令
  - [ ] 在 src/cli/commands.rs 中添加 prompt 子命令
  - [ ] 实现提示词列表和搜索功能
  - [ ] 实现提示词预览和使用功能
...
- [ ] 实现基础文件扫描功能
  - [ ] 在 src/knowledge/base.rs 中实现递归目录扫描函数 scan_directory
  - [ ] 实现文件类型过滤函数 filter_rust_files，仅支持 .rs 文件
  - [ ] 实现文件内容读取函数 read_file_content
- [ ] 实现基础代码索引功能
  - [ ] 添加 tantivy = "0.19" 依赖到 Cargo.toml
  - [ ] 在 src/knowledge/indexer.rs 中创建索引结构体 CodeIndex
  - [ ] 实现 create_index 函数创建 Tantivy 索引
  - [ ] 实现 add_file_to_index 函数将文件内容添加到索引
  - [ ] 实现 search_index 函数进行基础关键词搜索
- [ ] 实现 "codex index <path>" 命令
  - [ ] 在 src/cli/commands.rs 中添加 index 子命令
  - [ ] 在 src/main.rs 中实现 index 命令处理逻辑
  - [ ] 显示索引进度和基本统计信息
  - [ ] 实现索引结果的基本错误处理

### MVP.5 基础 AI 集成 (2-3 天)

- [ ] 实现单一 AI 平台支持（仅支持 OpenAI）
  - [ ] 添加 reqwest = { version = "0.11", features = ["json"] } 依赖到 Cargo.toml
  - [ ] 在 src/ai/adapter.rs 中创建 OpenAI 客户端结构体 OpenAIClient
  - [ ] 实现 new 函数初始化客户端，从环境变量读取 API 密钥
  - [ ] 实现 chat_completion 函数调用 OpenAI API
- [ ] 实现 "codex ask <question>" 命令
  - [ ] 在 src/cli/commands.rs 中添加 ask 子命令
  - [ ] 在 src/main.rs 中实现 ask 命令处理逻辑
  - [ ] 实现向 AI 发送问题并显示回答
  - [ ] 实现基本错误处理和重试机制
- [ ] 实现基础代码辅助功能
  - [ ] 在 src/cli/commands.rs 中添加 explain 和 generate 子命令
  - [ ] 在 src/main.rs 中实现 explain 命令处理逻辑，解释代码
  - [ ] 在 src/main.rs 中实现 generate 命令处理逻辑，生成代码
  - [ ] 实现基本的代码格式化显示

### MVP.6 基础 UI 界面 (3-4 天)

- [ ] 实现简单终端 UI
  - [ ] 添加 ratatui = "0.20" 和 crossterm = "0.26" 依赖到 Cargo.toml
  - [ ] 在 src/ui/terminal.rs 中创建基本 UI 结构体 TerminalUI
  - [ ] 实现 run 函数启动终端 UI
  - [ ] 实现基本的布局和组件
  - [ ] 实现基本的键盘输入处理
- [ ] 实现交互式问答界面
  - [ ] 在 src/ui/components/chat.rs 中实现问答界面布局
  - [ ] 实现问题输入区域和回答显示区域
  - [ ] 实现基本的命令历史记录
- [ ] 实现 "codex chat" 命令启动交互式界面
  - [ ] 在 src/cli/commands.rs 中添加 chat 子命令
  - [ ] 在 src/main.rs 中实现 chat 命令处理逻辑
  - [ ] 实现交互式界面的启动和退出
  - [ ] 实现基本的错误处理和状态管理
  - [ ] 在 src/ai.rs 中创建 OpenAI 客户端结构体 OpenAIClient
  - [ ] 实现 new 函数初始化客户端，从环境变量读取 API 密钥
  - [ ] 实现 chat_completion 函数调用 OpenAI API
- [ ] 实现 "codex ask <question>" 命令
  - [ ] 在 src/cli.rs 中添加 ask 子命令
  - [ ] 在 src/main.rs 中实现 ask 命令处理逻辑
  - [ ] 实现向 AI 发送问题并显示回答
  - [ ] 实现基本错误处理和重试机制
- [ ] 实现基础代码辅助功能
  - [ ] 在 src/cli.rs 中添加 explain 和 generate 子命令
  - [ ] 在 src/main.rs 中实现 explain 命令处理逻辑，解释代码
  - [ ] 在 src/main.rs 中实现 generate 命令处理逻辑，生成代码
  - [ ] 实现基本的代码格式化显示
...
- [ ] 实现简单终端 UI
  - [ ] 添加 ratatui = "0.20" 和 crossterm = "0.26" 依赖到 Cargo.toml
  - [ ] 在 src/ui.rs 中创建基本 UI 结构体 TerminalUI
  - [ ] 实现 run 函数启动终端 UI
  - [ ] 实现基本的布局和组件
  - [ ] 实现基本的键盘输入处理
- [ ] 实现交互式问答界面
  - [ ] 在 src/ui.rs 中实现问答界面布局
  - [ ] 实现问题输入区域和回答显示区域
  - [ ] 实现基本的命令历史记录
- [ ] 实现 "codex chat" 命令启动交互式界面
  - [ ] 在 src/cli.rs 中添加 chat 子命令
  - [ ] 在 src/main.rs 中实现 chat 命令处理逻辑
  - [ ] 实现交互式界面的启动和退出
  - [ ] 实现基本的错误处理和状态管理
  - [ ] 实现基本的帮助信息显示
...
- [ ] 实现配置文件支持
  - [ ] 添加 toml = "0.7" 依赖到 Cargo.toml
  - [ ] 在 src/config.rs 中扩展 Config 结构体，添加所有配置字段
  - [ ] 实现从文件加载配置的 load_config 函数
  - [ ] 实现配置文件路径自动查找（当前目录、用户主目录）
  - [ ] 实现环境变量覆盖配置功能
- [ ] 扩展配置项
  - [ ] 添加 AI 平台配置（API 密钥、模型选择）
  - [ ] 添加知识库配置（索引路径、排除规则）
  - [ ] 添加 UI 配置（主题、颜色方案）
- [ ] 实现配置验证
  - [ ] 实现基本配置项验证
...
- [ ] 实现详细错误类型
  - [ ] 在 src/error.rs 中扩展 AppError 枚举，添加所有错误类型
  - [ ] 实现每种错误类型的上下文信息
  - [ ] 实现用户友好的错误信息显示
  - [ ] 实现开发者调试信息显示
- [ ] 实现错误链追踪
  - [ ] 添加 eyre = "0.6" 依赖到 Cargo.toml
  - [ ] 使用 eyre 实现错误链追踪
  - [ ] 实现错误源的自动识别和标记
  - [ ] 实现错误传播路径的可视化显示
...
- [ ] 更新索引和搜索功能
  - [ ] 修改 src/knowledge_base.rs 中的索引功能，支持多语言
  - [ ] 更新搜索功能，支持语言过滤
  - [ ] 实现语言特定的代码高亮显示
